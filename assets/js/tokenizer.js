// tokensにインデックスを振る
var tokenizer = function(lines, nonPassList, debug) {
	var tokens        = [],
		openPareStack = [],
		analysis      = {},
		dataObject    = {},
	...
	*/
	function getPrevChar(pushed, j, k) {
		
		// 空白以外の文字を探す
		// kがゼロになった場合はjを--
		// jがゼロの場合は j0 k0の値を返す
		// 
		var res = pushed.length ? pushed[pushed.length-1].value : ' ';
	...
	function typeCheck(token, pushed, j, k) {
		var res       = token[j].type,
			value     = token[j].value[k],
			prevTokenType = pushed.length
								? pushed[pushed.length-1].type
								: token[j].type;
		...
		if ( !res.match('string') ) {
			...
			// その他のホワイトスペース
				...
				debug && console.log('prev: ' + getPrevChar(token, j, k), 'current: ' + token[j].value[k], 'next: ' + getNextChar(token, j, k));
				
				var prevChar = getPrevChar(pushed, j, k),
	...
	// try {
		...
		for (var i = 0, len = lines.length-1; i < len; i++) {
			...
			for (var j = 0, jlen = lines[i].length; j < jlen; j++, tokenCnt++) {
				...
				debug && console.log('●: ' +  value + '[ ' + lines[i][j].type + ' ]');
				/*
				debug && postMessage({
					'type': 'log',
				...
				for (var k = 0, klen = value.length; k < klen; k++) {
					...
					type     = typeCheck(lines[i], tokens, j, k);
					...
					debug && console.log('○: ' + lines[i][j].value[k] + '[ ' + analysis.type + ' ]');
					/*
					debug && postMessage({
						'type': 'log',
		...
		return [ tokens, startup, enteredNum, idxMap, createKeywordMap( tokens ), arrInput ];
/*
	} catch (e) {
